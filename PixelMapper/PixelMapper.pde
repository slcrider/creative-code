...
final color[] COLORS = {color(0,0,0),
                        ...
                        color(0,255,0),
                        color(0,0,255)};
int[] color_index = new int[8];
Pixel[] pixel_map = new Pixel[8];
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            if ((i % 2 == 0 && j % 2 == 0) || (i % 2 == 1 && j % 2 == 1 ))  {
                pixel_map[counter] = new Pixel(pixel_size, i * (width / 4), j * (height / 4));
                color_index[counter] = int(random(0, COLORS.length));
                pixel_map[counter].setColor(COLORS[color_index[i + 4 * j]]);
            }
        }
    }
...
void draw() {
    ...
    for (int i = 0; i < pixel_map.length; i++) {
        pixel_map[i].render();
    }
...
void updateColor(int index) {
    ...
    pixel_map[index].setColor(COLORS[color_index[index]]);
}